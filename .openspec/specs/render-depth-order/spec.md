# render-depth-order Specification
...
### Requirement: Moving characters are Y-sorted continuously
The system SHALL assign the player, enemies, and moving NPCs a shared layer order derived from their current world Y position whenever their movement update changes or evaluates their position.

#### Scenario: Moving characters cross vertically
- **WHEN** two characters move past one another at different world Y positions
- **THEN** each character SHALL recalculate its layer order during its movement update and the lower on-screen ground contact SHALL render in front
