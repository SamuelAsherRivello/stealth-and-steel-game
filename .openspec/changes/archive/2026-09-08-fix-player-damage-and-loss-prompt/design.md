...
Preserve the enemy damage, arrow, knockback and 250 ms death contracts. Later knife combat supersedes player contact damage. The current loss flow retains the defeated player record for verified BIS revival, shows Pay 1000 Sats To Continue and Restart Game, and preserves the world on paid continuation. Earlier Continue/reload-only UI and unconditional actor-removal descriptions below are historical. The synchronized player-loss-flow delta carries the current defeat and loss-choice requirements; its main spec also governs payment matching, cleanup and late callbacks.
