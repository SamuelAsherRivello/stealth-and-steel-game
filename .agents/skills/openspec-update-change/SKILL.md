...
**Canonical identity:** Accept either a change name or `C###` ID, while using
the readable name as the preferred interface. Preserve the change ID and all
existing `C###-T###` task IDs. Newly added tasks receive the next unused task
ID for the parent change; reordering or editing never changes an existing ID.
...
`$openspec-continue-change (Codex) or /openspec-continue-change (other agents)` is an optional workflow and may not be installed. Before suggesting it anywhere below, verify that it is available. If it is unavailable, `npm run openspec -- status --change "<name>" --json` shows the next artifact and `npm run openspec -- instructions "<artifact-id>" --change "<name>" --json` explains how to create it.
...
1. **Select the change**
   ...
   - If ambiguous, run `npm run openspec -- list --json` to get available changes sorted by most recently modified, and ask the user to select one
...
- Use the artifact ids and paths reported by `npm run openspec -- status`; never branch on hardcoded artifact names.
