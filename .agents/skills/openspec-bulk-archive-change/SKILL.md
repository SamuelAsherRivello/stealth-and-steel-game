...
1. **Get active changes**
   ...
   Run `npm run openspec -- list --json` to get all active changes.
...
2. **Prompt for change selection**
   ...
   **Load current archive inputs once for the selected root before batch validation:**
   ...
   `npm run openspec -- instructions archive --change "<selected-change>" --json` with the
...
3. **Batch validation - gather status for all selected changes**
   ...
   a. **Artifact status** - Run `npm run openspec -- status --change "<name>" --json`
...
7. **Confirm batch operation**
   ...
   `npm run openspec -- instructions specs --change "<name>" --json` exactly once with the
...
8. **Execute archive for each confirmed change**
   ...
   c. **Perform the archive**:
      ...
      Target name: use the change name as-is when it already starts with a `YYYY-MM-DD-` prefix; otherwise prepend the current date as `YYYY-MM-DD-<name>` (same rule as `npm run openspec -- archive`).
