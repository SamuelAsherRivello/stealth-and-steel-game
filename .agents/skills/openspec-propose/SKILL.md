...
2. **Determine the workflow schema**
   ...
   **Use a different schema only if the user:**
   ...
   - Asks to "show workflows" or asks "what workflows" exist → resolve the authoritative root by running `npm run openspec -- context --json` from the current working directory. If the user explicitly selected a registered store, use `npm run openspec -- context --json --store "<store-id>"`. Then run `npm run openspec -- schemas --json` with its working directory set to the returned `root.path` and let them choose. This preserves roots selected by a local `store:` pointer or the global `defaultStore`; when a registered store was explicitly selected, append `--store "<store-id>"` to `npm run openspec -- schemas --json` as well. If context reports only `no_openspec_root`, run `npm run openspec -- schemas --json` from the current working directory instead. Do not use this fallback for invalid or unavailable stores.
...
3. **Create the change directory**
   ...
   ```bash
   npm run openspec -- new change "<name>"
   ```

   Using an explicitly requested schema:
   ```bash
   npm run openspec -- new change "<name>" --schema "<schema-name>"
   ```
...
5. **Create every artifact in the required set**
   ...
   a. **For each artifact that is `ready` (dependencies satisfied)**:
      - Get instructions:
        ```bash
        npm run openspec -- instructions <artifact-id> --change "<name>" --json
        ```
   ...
   b. **Continue until every artifact in the required set exists (not just `apply.requires`)**
      - After creating each artifact, re-run `npm run openspec -- status --change "<name>" --json`
...
6. **Show final status**
   ```bash
   npm run openspec -- status --change "<name>"
   ```
...
- Follow the `instruction` field from `npm run openspec -- instructions` for each artifact type - it is the authoritative guidance, even for familiar artifact names
