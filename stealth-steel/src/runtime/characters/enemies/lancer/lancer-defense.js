...
export const DEFAULT_LANCER_DEFENSE_CONFIG = Object.freeze({
...
export function createLancerDefenseConfig(overrides = {}) {
  return Object.freeze({
    reactionLookaheadSeconds: Math.max(0, finiteOr(
      overrides.reactionLookaheadSeconds,
      DEFAULT_LANCER_DEFENSE_CONFIG.reactionLookaheadSeconds,
    )),
    defenseDurationSeconds: Math.max(0, finiteOr(
      overrides.defenseDurationSeconds,
      DEFAULT_LANCER_DEFENSE_CONFIG.defenseDurationSeconds,
    )),
  });
}
...
export function selectIncomingProjectile(
  ...
  for (const projectile of projectiles) {
    ...
    if (projectile.direction.x === -lancerFacing) return projectile;
  }
  return null;
}
