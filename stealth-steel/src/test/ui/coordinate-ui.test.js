...
test("Coordinates UI lives in src/ui and contains separate pixel and grid output lines", async () => {
  ...
  assert.match(css, /\.gold-counter,\s*\.level-counter,\s*\.items-counter,\s*\.coordinates-ui\s*\{/);
  assert.match(css, /\.items-counter\s*\{[^}]*top:\s*calc\(var\(--ui-safe-top\) \+ 3em\);/s);
  assert.match(css, /\.coordinates-ui\s*\{[^}]*top:\s*calc\(var\(--ui-safe-top\) \+ 4\.5em\);/s);
