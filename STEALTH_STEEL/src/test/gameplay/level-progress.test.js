...
test('continue persists level, final omits next, restart preserves wallet',()=>{
  ...
  assert.deepEqual(JSON.parse(values.get(RUN_STORAGE_KEY)),{order:[1,2],completed:0,pendingTransition:true});assert.equal(values.get('wallet'),'untouched');assert.equal(reloads,2);
});
