...
export function createWarrior({
  ...
  const layers = {};
  const sprites = {};
  const getArtScreenPosition = (worldPosition) => worldToScreen({ x: worldPosition.x + WARRIOR_ART_OFFSET.x, y: worldPosition.y + WARRIOR_ART_OFFSET.y }, 1, (bounds.renderHeight ?? bounds.height));
